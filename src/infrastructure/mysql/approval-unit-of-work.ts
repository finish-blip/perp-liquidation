...
import { MysqlApprovalRepository } from "./approval-repository.js";
...
export class MysqlApprovalUnitOfWork implements ApprovalUnitOfWork {
  constructor(private readonly pool: Pool) {}

  async execute<T>(
    handler: (repositories: ApprovalRepositories) => Promise<T>
  ): Promise<T> {
    return withTransaction(this.pool, async (connection) =>
      handler({
        approvals: new MysqlApprovalRepository(connection),
